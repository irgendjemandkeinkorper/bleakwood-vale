  ...
  $('scr-online-entry').innerHTML = `
    ...
    <div class="pgrid" style="grid-template-columns:repeat(auto-fit,minmax(320px,1fr));max-width:900px;margin:0 auto">
      <div class="panel">
        ...
        <select id="oe-hook">${HOOKS.map((h,i)=>`<option value="${i}">${esc(h.title)}</option>`).join('')}</select>
...
export async function onlineCreateRoom(){
  try{
    ...
    const code = await createRoom(hook, name);
    enterRoom(code);
  } catch(err){ fail(err); }
}
...
function renderOnlineScene(room){
  const c = room.current, p = room.players[c.starter], a = room.archetypes[c.archIdx];
  const mySeat = mySeatIndex(room);
